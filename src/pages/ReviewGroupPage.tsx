    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-white/[0.08] pb-5">
        <div>
          <Link
            to="/groups"
            className="mb-2 text-xs font-semibold text-blue-400 hover:underline inline-flex items-center gap-1"
          >
          ...
          <p className="mt-1 text-xs text-slate-400">
            {group.files.length} candidate files · {formatBytes(group.recoverable)} reclaimable storage
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
              ...
              return (
                ...
                >
                  <input
                    ...
                    className="h-4 w-4 rounded border-white/[0.2] bg-black/40 text-blue-600 focus:ring-blue-500 cursor-pointer shrink-0 accent-blue-600"
                  />
        ...
        <div className="space-y-4">
          <Card className="p-6">
            ...
            <div className="space-y-2">
              {group.signals.map(signal => (
                <div
                  key={signal.label}
                  className="flex items-center justify-between rounded-xl border border-white/[0.08] bg-white/[0.03] p-2.5 text-xs"
                >
                  <span className="text-slate-300 text-[11px]">{signal.label}</span>
                  {signal.score ? (
                    <span className="font-mono font-bold text-blue-400 text-xs">{signal.score}%</span>
                  ) : (
          ...
          <Card className="p-6 bg-gradient-to-br from-emerald-500/10 via-white/[0.03] to-transparent border-emerald-500/30">
            ...
            <div className="mt-4 border-t border-emerald-500/20 pt-3 text-[11px] text-emerald-300/80">
              Zero risk: Quarantined files are staged for 30 days and can be restored at any time.
            </div>
          </Card>
        </div>
      </div>
